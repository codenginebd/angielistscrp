...
class Parser:
	...
	def parse_basic_info(page):
		...
		for each_td in sr_info_tds:
			...
			if basic_info['rating'] and basic_info['rating'] == 'A':
		...
		return (basic_info_list,_next_button_available)
	...
	def parse_address(page):
		soup = BeautifulSoup(page)
		address_element_span = soup.find('span',{'id':'ctl00_ContentPlaceHolderMainContent_SPSideBar1_ContactAddress'})
		print address_element_span.contents[1]
		# if address_element_span:
			# address_element_content1 = address_element_span.contents[0].string if len(address_element_span.contents) > 0 else ''
			# if address_element_content1:
				# address_info['a1'] = address_element_content1
			# address_element_content2 = address_element_span.contents[2].string if len(address_element_span.contents) > 2 else ''
			# if address_element_content2:
				# address_info['a2'] = address_element_content2
				# address_info['city'],address_info['state'],address_info['zip'] = Parser.extract_address(address_element_content2)
		# phone_number_span_element = soup.find('span',{'id':'ctl00_ContentPlaceHolderMainContent_SPSideBar1_PhoneNumbers'})
